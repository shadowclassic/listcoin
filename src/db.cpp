...
{
	LogPrintf("Beginning CDBEnv::Open %s\n",pathEnv_);
    ...
    pdb(NULL), activeTxn(NULL)
{
...

    {
        ...
        if (pdb == NULL)
        {
			...
			LogPrintf("After New DB\n");
            bool fMockDb = bitdb.IsMock();
			LogPrintf("After IsMock\n");
            if (fMockDb)
            {
				LogPrintf("fMockDB\n");
            ...
            if (ret != 0)
            {
                ...
                --bitdb.mapFileUseCount[strFile];
                strFile = "";
				LogPrintf("CDB : Error %d, can't open database %s\n", ret, strFile.c_str());
	...
	LogPrintf("DB Constructor completed\n");
}
