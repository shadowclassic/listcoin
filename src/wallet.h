...
public:
    ...
    int64_t GetListDebit(const CTxIn& txin) const;
    int64_t GetListCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nLIC, int64_t& nList) const
    {
        nLIC = 0;
        nList = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nList += GetListCredit(txout);
            } else
                nLIC += GetCredit(txout);
            if (!MoneyRange(nLIC)
                || !MoneyRange(nList))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableListCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableListCreditCached;
    
    mutable int64_t nCredLICCached;
    mutable int64_t nCredListCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableListCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredLIC, int64_t& nCredList, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredLIC = nCredList = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredLICCached = 0;
            nCredListCached = 0;
            pwallet->GetCredit(*this, nCredLICCached, nCredListCached);
            fCreditSplitCached = true;
        };
        
        nCredLIC = nCredLICCached;
        nCredList = nCredListCached;
        return true;
    }
    ...
    int64_t GetAvailableListCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableListCreditCached)
            return nAvailableListCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableListCredit() : value out of range");
            };
        };
        
        nAvailableListCreditCached = nCredit;
        fAvailableListCreditCached = true;
        return nCredit;
    };
