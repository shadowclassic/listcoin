...
#include "listbridge.moc"

ListBridge::ListBridge(ListGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void ListBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void ListBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void ListBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void ListBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void ListBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void ListBridge::populateOptions()
{
...
// Transactions
bool ListBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void ListBridge::clearRecipients()
{
    recipients.clear();
}

bool ListBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 LIC, 1 List
        switch(rcp.txnTypeInd)
        {
            case TXT_LIC_TO_LIC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_LIC_TO_ANON:
                formatted.append(tr("<b>%1</b> to SHADOW %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to SHADOW %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_LIC:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to LIC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsListBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your list balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void ListBridge::openCoinControl()
{
...
void ListBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void ListBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap ListBridge::listAnonOutputs()
{
...
void ListBridge::populateTransactionTable()
{
...
void ListBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void ListBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString ListBridge::transactionDetails(QString txid)
{
...
// Addresses
void ListBridge::populateAddressTable()
{
...
void ListBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void ListBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString ListBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString ListBridge::lastAddressError()
{
    QString sError;
...
QString ListBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void ListBridge::updateAddressLabel(QString address, QString label)
{
...
bool ListBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool ListBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void ListBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void ListBridge::appendMessage(int row)
{
...
void ListBridge::populateMessageTable()
{
...
void ListBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool ListBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool ListBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString ListBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);;
}

bool ListBridge::setPubKey(QString address, QString pubkey)
{
...
bool ListBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QString ListBridge::createGroupChat(QString label)
{
...
QString ListBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList ListBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void ListBridge::connectSignals()
{
...
QVariantMap ListBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap ListBridge::listLatestBlocks()
{
...
QVariantMap ListBridge::findBlock(QString searchID)
{
...
QVariantMap ListBridge::blockDetails(QString blkHash)
{
...
QVariantMap ListBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap ListBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap ListBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap ListBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap ListBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap ListBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap ListBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap ListBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap ListBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Listcoin prefix.");
            return result;
        }
...
QVariantMap ListBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap ListBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap ListBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
