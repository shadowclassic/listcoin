...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return ListGUI::tr("Received with");
    case RecvFromOther:
        return ListGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return ListGUI::tr("Sent to");
    case SendToSelf:
        return ListGUI::tr("Payment to yourself");
    case Generated:
        return ListGUI::tr("Mined");
    case RecvList:
        return ListGUI::tr("Received list");
    case SendList:
        return ListGUI::tr("Sent list");
    case Other:
        return ListGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvList:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendList:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredLIC, nCredList;
    wtx.GetCredit(nCredLIC, nCredList, true);
    int64_t nCredit = nCredLIC + nCredList;
    ...
    {
        if (nNet > 0 && nCredList > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvList, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendList, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvList;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv list
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendList;
                    sub.address = CBitcoinAddress(ckidD).ToString();
