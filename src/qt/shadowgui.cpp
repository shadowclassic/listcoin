// Copyright (c) 2014 The ListCoin developers
...
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/list"));
    setWindowIcon(QIcon(":icons/list"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/list/index.html");
#else
    QFile html("/opt/list/index.html");
#endif
...
void ListGUI::pageLoaded(bool ok)
{
...
void ListGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void ListGUI::urlClicked(const QUrl & link)
{
...
void ListGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/list"), tr("&About ListCoin"), this);
    aboutAction->setToolTip(tr("Show information about ListCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for ListCoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/list"), tr("&Show / Hide"), this);
...
void ListGUI::setClientModel(ClientModel *clientModel)
{
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/list_testnet"));
            setWindowIcon(QIcon(":icons/list_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/list_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Umbra client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/list_testnet"));
...
void ListGUI::setWalletModel(WalletModel *walletModel)
{
...
void ListGUI::setMessageModel(MessageModel *messageModel)
{
...
void ListGUI::createTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon(":/icons/list"));
...
#ifndef Q_OS_MAC
void ListGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ListGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to ListCoin network", "", count));
}

void ListGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void ListGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void ListGUI::changeEvent(QEvent *e)
{
...
void ListGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void ListGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::LIC, nFeeRequired));
...
void ListGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void ListGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void ListGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void ListGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ListGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void ListGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ListCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void ListGUI::handleURI(QString strURI)
{
...
void ListGUI::setEncryptionStatus(int status)
{
...
void ListGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void ListGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void ListGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ListGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void ListGUI::updateStakingIcon()
{
