...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uLIC",
                this.display = "&micro;LIC";
                break;

            case 3:
                this.name    = "sLIC",
                this.display = "Listshi";
                break;

            default:
                this.name = this.display = "LIC";
        }
...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"http://ajax.googleapis.com/ajax/services/feed/load?v=2.0\&q=https://blog.listproject.io/rss/",
            dataType: 'jsonp'
    ...
    updateBalance: function(balance, listBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            listBal   = this.listBal  .data("orig"),
            ...
            this.listBal  .data("orig", listBal),
        ...
        this.formatValue("listBal",   listBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a List address (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="List"                    ' + (unit.type == 0 ? "selected" : "") + '>LIC</option> \
                    <option value="1" title="Milli-List (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mLIC</option> \
                    <option value="2" title="Micro-List (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;LIC</option> \
                    <option value="3" title="Listshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Listshi</option> \
                </select> \
            </div> \
...
function changeTxnType()
{
    ...
    {
        $("#tx_ringsize,#suggest_ring_size,#spend_list").hide();
